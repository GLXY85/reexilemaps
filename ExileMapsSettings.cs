...
public class WaypointSettings
{
    ...
    [Menu("Route Settings", 1000, CollapsedByDefault = false)]
    public RoutingSettings RoutingSettings { get; set; } = new RoutingSettings();
...
{
    [Menu("Enable Route Pathfinding")]
    public ToggleNode EnableRoutes { get; set; } = new ToggleNode(true);
    
    [Menu("Auto-update Route")]
    public ToggleNode AutoUpdateRoute { get; set; } = new ToggleNode(true);
    
    [Menu("Route Update Interval (seconds)")]
    public RangeNode<float> RouteUpdateInterval { get; set; } = new RangeNode<float>(1.0f, 0.1f, 5.0f);
    
    [Menu("Route Line Width")]
    public RangeNode<float> RouteLineWidth { get; set; } = new RangeNode<float>(4.0f, 1.0f, 10.0f);
    
    [Menu("Route Line Color")]
    public ColorNode RouteLineColor { get; set; } = new ColorNode(Color.FromArgb(220, 255, 165, 0)); // Оранжевый
    
    [Menu("Route Start Color")]
    public ColorNode RouteStartColor { get; set; } = new ColorNode(Color.FromArgb(220, 100, 180, 255)); // Светло-синий
    
    [Menu("Route End Color")]
    public ColorNode RouteEndColor { get; set; } = new ColorNode(Color.FromArgb(220, 0, 60, 180)); // Насыщенный синий
    
    [Menu("Use Gradient")]
    public ToggleNode UseGradient { get; set; } = new ToggleNode(true);
    
    [Menu("Show Direction Arrows")]
    public ToggleNode ShowDirectionArrows { get; set; } = new ToggleNode(true);
    
    [Menu("Использовать заблокированные карты")]
    public ToggleNode UseLockedNodesInRoute { get; set; } = new ToggleNode(true);
    
    [Menu("Arrow Size")]
    public RangeNode<float> ArrowSize { get; set; } = new RangeNode<float>(10.0f, 5.0f, 30.0f);
}
